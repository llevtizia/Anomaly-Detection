...
\begin{itemize}
    ...
    \item \textbf{Autoencoder} (MSE/SSIM): 80-85\% image AUROC (problemi di reconstruction bias)
    \item \textbf{CNN Feature Dictionary}: performance migliori ma ancora limitate
...
\end{itemize}
Queste performance modeste dimostrarono la necessità di approcci più sofisticati.

\paragraph{Metriche Standardizzate.} Il dataset introdusse e popolarizzò due metriche fondamentali:
\begin{itemize}
    \item \textbf{AUROC (Area Under ROC Curve)}: per valutazione a livello immagine e pixel
    \item \textbf{PRO (Per-Region Overlap)}: sviluppata appositamente per compensare il bias dell'AUROC pixel-level verso difetti di grandi dimensioni, assegnando uguale peso a regioni anomale di qualsiasi dimensione
\end{itemize}

\paragraph{Impatto Profondo sulla Ricerca.} MVTec AD ha catalizzato il progresso del campo:
\begin{enumerate}
    \item Fornì il primo benchmark realistico, riproducibile e completo
    \item Espose i limiti dei metodi contemporanei (GAN-based, autoencoder)
    \item Motivò lo sviluppo di nuove generazioni di metodi (feature-based, memory-based, student-teacher) che oggi superano il 99\% AUROC
